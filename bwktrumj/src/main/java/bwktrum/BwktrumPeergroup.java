...
import bwktrum.exceptions.InvalidPeerVersion;
import bwktrum.listeners.AddressListener;
import bwktrum.listeners.PeerDataListener;
import bwktrum.listeners.PeerListener;
 ...
 *
 * Class in charge of manage the connection with bwktrum servers.
 *
...
public class BwktrumPeergroup implements PeerListener, PeerDataListener {

    private static final Logger log = LoggerFactory.getLogger(BwktrumPeergroup.class);
    ...
    /** Trusted peer */
    private BwktrumPeer trustedPeer;
    // Currently active peers.
    private final CopyOnWriteArrayList<BwktrumPeer> peers;
    // Currently connecting peers.
    private final CopyOnWriteArrayList<BwktrumPeer> pendingPeers;
    ...
    public BwktrumPeergroup(NetworkConf networkConf, WalletManager walletManager, AddressStore addressStore) throws IOException {
    ...
    public BwktrumPeergroup(NetworkConf networkConf) throws IOException {
        this.peers = new CopyOnWriteArrayList<>();
        this.pendingPeers = new CopyOnWriteArrayList<>();
        this.networkConf = networkConf;
        this.ioManager = new IoManager(1,1);
    ...
    public synchronized void start() throws InterruptedException, ConnectionFailureException {
        try {
            log.info("Starting BwktrumPeergroup");
            isActive = true;
            // todo: first part discovery..
            /*
            * Connect to the trusted node and get servers from it.
            */
            trustedPeer = new BwktrumPeer(networkConf.getTrustedServer(), ioManager,versionMsg);
            ...
            isRunning = false;
            isActive = false;
            log.error("BwktrumPeerGroup start",e);
            throw e;
        }
    }
    ...
    @Override
    public void onConnected(BwktrumPeer bwktrumPeer) {
        try {
            if (bwktrumPeer == trustedPeer) {
                ...
                for (PeerListener peerConnectionListener : peerConnectionListeners) {
                    peerConnectionListener.onConnected(bwktrumPeer);
                }
                ...
                for (InetSocketAddress inetSocketAddress : networkConf.getNetworkServers()) {
                    BwktrumPeerData peerData = new BwktrumPeerData(inetSocketAddress.getHostName(),inetSocketAddress.getPort(),0);
                    BwktrumPeer peer = new BwktrumPeer(peerData,ioManager,versionMsg);
                ...
                log.info("Non trusted peer connected, "+bwktrumPeer.getPeerData());
                pendingPeers.remove(bwktrumPeer);
                peers.add(bwktrumPeer);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    @Override
    public void onDisconnected(BwktrumPeer bwktrumPeer) {

    }

    @Override
    public void onExceptionCaught(BwktrumPeer bwktrumPeer, Exception e) {
    ...
    @Override
    public void onSubscribedAddressChange(BwktrumPeer bwktrumPeer, String address, String status) {
        try {
    ...
    @Override
    public void onListUnpent(BwktrumPeer bwktrumPeer,String address, List<Unspent> unspents) {
    ...
    @Override
    public void onBalanceReceive(BwktrumPeer bwktrumPeer, String address, long confirmed, long unconfirmed) {
        try {
            if (bwktrumPeer == trustedPeer) {
    ...
    @Override
    public void onGetHistory(BwktrumPeer bwktrumPeer, StatusHistory statusHistory) {
        try {
