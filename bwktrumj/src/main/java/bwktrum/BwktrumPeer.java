...
import bwktrum.messages.GetHeader;
import bwktrum.messages.GetHistoryMsg;
import bwktrum.messages.GetTxMsg;
import bwktrum.messages.ListUnspentMsg;
...
public class BwktrumPeer implements IoHandler{

    private final Logger log;

    /** Peer data */
    private BwktrumPeerData peerData;
    /**  */
    ...
    public BwktrumPeer(BwktrumPeerData peerData,IoManager ioManager,VersionMsg versionMsg) {
        this.peerData = peerData;
        this.ioManager = ioManager;
        this.versionMsg = versionMsg;
        this.log = LoggerFactory.getLogger(BwktrumPeer.class.getName()+"-"+peerData.getHost());
    }
    ...
    public void connect() throws ConnectionFailureException, InterruptedException {
        if (isInitilizing.compareAndSet(false,true) && !isRunning.get()) {
            ...
            throw new IllegalStateException("BwktrumPeer already initializing");
        }
    }
    ...
    public BwktrumPeerData getPeerData() {
        return peerData;
    }
